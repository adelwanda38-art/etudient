...
async def summary_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تحقق هل مستخدم موثق أو استخدم المجاني
...
async def receive_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ تم استلام الملف {len(pdfs)}/4\n{'اكتب /done للمتابعة' if len(pdfs) >= 1 else 'أرسل المزيد أو اكتب /done'}"
    )
    return WAITING_PDF
...
async def done_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # تقسيم الرسالة إذا كانت أطول من 4000 حرف
...
ADMIN_ID = 123456789  # ضع ID حسابك في تلغرام هنا
...
async def change_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إعادة تعيين المستخدمين المسجلين (اختياري)
    save_data(data)
